<?php
/**
 * 僑光科大招生查詢系統 (Gemini RAG-based) - 調試版本
 * 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['query'])) {
    ...
    if (!empty($query)) {
        ...
        try {
            // 獲取相關文檔
            $relevantDocs = $ragSystem->retrieveDocuments($query, 3);
            $debugInfo['relevant_docs'] = $relevantDocs;
            
            // 處理查詢
            $answer = $ragSystem->processQuery($query);
            $debugInfo['answer'] = $answer;
            
        } catch (Exception $e) {
            $debugInfo['error'] = $e->getMessage();
            $debugInfo['trace'] = $e->getTraceAsString();
            $answer = "發生錯誤: " . $e->getMessage();
        }
    }
}
...
try {
    ...
    $result = $db->query('SELECT doc_id, title, SUBSTR(content, 1, 100) as content_preview FROM documents LIMIT 10');
    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
        $dbInfo[] = $row;
    }
    $db->close();
...
<head>
    ...
    <title>僑光科大招生查詢系統 - Gemini RAG (調試模式)</title>
            ...
            <section class="query-section">
                ...
                <div class="debug-info">
                    ...
                    <div class="debug-section">
                        <h4>查詢:</h4>
                        <pre><?php echo htmlspecialchars(print_r($debugInfo['query'] ?? '', true)); ?></pre>
                    </div>
                    
                    <?php if (isset($debugInfo['relevant_docs'])): ?>
                    <div class="debug-section">
                        <h4>找到的相關文檔:</h4>
                        <pre><?php echo htmlspecialchars(print_r($debugInfo['relevant_docs'], true)); ?></pre>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="debug-section">
                        <h4>錯誤:</h4>
                        <pre><?php echo htmlspecialchars($debugInfo['error']); ?></pre>
                        <h4>堆棧追踪:</h4>
                        <pre><?php echo htmlspecialchars($debugInfo['trace']); ?></pre>
                    </div>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
                ...
                <div class="debug-info">
                    <h3>資料庫內容預覽</h3>
                    <pre><?php echo htmlspecialchars(print_r($dbInfo, true)); ?></pre>
                </div>
            </section>
